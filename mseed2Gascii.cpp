...
//static double g_dwCalibration	   = 0.0003110039;
//
...
CMseed2Gascii::CMseed2Gascii()
{
	verbose      = 0;    /* Verbosity level */
	reclen       = -1;   /* Record length, -1 = autodetected */
	deriverate   = 0;    /* Use sample rate derived instead of the reported rate */
	indifile     = 0;    /* Individual file processing flag */
	memset(unitsstr, 0x00, sizeof(unitsstr));
	strcpy(unitsstr, "Counts");
	//*unitsstr     = "Counts"; /* Units to write into output headers */
	outputfile   = 0;    /* Output file name for single file output */
	ofp          = 0;    /* Output file pointer */
	outformat    = 4;    /* Output file format */
	slistcols    = 1;    /* Number of columns for sample list output */
	timetol      = -1.0; /* Time tolerance for continuous traces */
	sampratetol  = -1.0; /* Sample rate tolerance for continuous traces */
	g_dwCalibration	   = 0.0003110039;
...
int CMseed2Gascii::MakeMseed2Gascii (char* inputFile, char* outputFile, double dwCalib)
{
	MSTraceGroup *mstg = 0;
	MSTrace *mst;
	MSRecord *msr = 0;
	...
	g_dwCalibration = dwCalib;
  
	/* Process given parameters (command line and parameter file) */
	//if( parameter_proc (argc, argv) < 0 )
	if( parameter_proc(inputFile) < 0 )
		return -1;

	outputfile = new char[strlen(outputFile) + 1];
	memset(outputfile, 0x00, strlen(outputFile) + 1);
	memcpy(outputfile, outputFile, strlen(outputFile));
 ...
 ***************************************************************************/
int64_t CMseed2Gascii::writeascii (MSTrace *mst)
{
	BTime btime;
	char outfile[1024];
	...
	* "TIMESERIES Net_Sta_Loc_Chan_Qual, ## samples, ## sps, isotime, SLIST|TSPAIR, INTEGER|FLOAT|ASCII, Units" */
	...
	else if( outformat == 3 )
    {
		...
		/* Print header line */
		fprintf(ofp, "TIMESERIES %s, %lld samples, %g sps, %s, TSPAIR, %s, %s, gal(cm/s^2)\n"
					, srcname, (long long int)mst->numsamples, mst->samprate, timestr, samptype, unitsstr);
		...
		for( cnt = 0; cnt < mst->numsamples; cnt++ )
		{
			...
			// dskim
// fprintf(stderr, "002\n");
			dwDataTime = (double)(samptime / (HPTMODULUS * 1.0));//  MS_HPTIME2EPOCH(samptime);
			// dwDataTime = samptime / (HPTMODULUS * mst->samprate);//  MS_HPTIME2EPOCH(samptime);
// fprintf(stderr, "003\n");
			...
			{
// fprintf(stderr, "004\n");
				fprintf(ofp, "%s %.2f %d %.8g\n", timestr, dwDataTime, *(int32_t *)sptr, (*(int32_t *)sptr) * g_dwCalibration);
// fprintf(stderr, "005\n");
			}
			else if( mst->sampletype == 'f' )
			{
				fprintf(ofp, "%s %.2f %.8g %.8g\n", timestr, dwDataTime, *(float *)sptr, (*(int32_t *)sptr) * g_dwCalibration);
			}
			else if( mst->sampletype == 'd' )
			{
				fprintf(ofp, "%s %.2f %.10g %.8g\n", timestr, dwDataTime, *(double *)sptr, (*(int32_t *)sptr) * g_dwCalibration);
			}
			samptime += hpdelta;
		}
    }
	else if( outformat == 4 )		// RungeKutta 알고리즘 사용을 위한 time 변환 데이터 추가
	{
		hptime_t samptime = mst->starttime;
		hptime_t hpdelta = ( mst->samprate ) ? (hptime_t) (HPTMODULUS / mst->samprate) : 0;
		...
		/* Print header line */
		fprintf(ofp, "TIMESERIES %s, %lld samples, %g sps, %s, TSPAIR, %s, %s, gal(cm/s^2), time(RungeKutta)\n"
			, srcname, (long long int)mst->numsamples, mst->samprate, timestr, samptype, unitsstr);
		...
		for( cnt = 0; cnt < mst->numsamples; cnt++ )
		{
			...
			if( mst->sampletype == 'i' )
			{
				fprintf(ofp, "%s %.2f %d %.8g %.2f\n", timestr, dwDataTime, *(int32_t *)sptr, (*(int32_t *)sptr) * g_dwCalibration, dwDataTime - dblBaseTime);
			}
			else if( mst->sampletype == 'f' )
			{
				fprintf(ofp, "%s %.2f %.8g %.8g %.2f\n", timestr, dwDataTime, *(float *)sptr, (*(int32_t *)sptr) * g_dwCalibration, dwDataTime - dblBaseTime);
			}
			else if( mst->sampletype == 'd' )
			{
				fprintf(ofp, "%s %.2f %.10g %.8g %.2f\n", timestr, dwDataTime, *(double *)sptr, (*(int32_t *)sptr) * g_dwCalibration, dwDataTime - dblBaseTime);
			}
			samptime += hpdelta;
		}
	}
 ...
 ***************************************************************************/
//static int parameter_proc (int argcount, char **argvec)
int CMseed2Gascii::parameter_proc(char* sFileName)
{
	//int optind;
 // 
 ...
 //   }
	...
	/* Add the file name to the intput file list */
	if( ! addnode(&filelist, NULL, 0, sFileName, strlen(sFileName)+1) )
	{
		fprintf(stderr, "Error adding file name to list\n");
	}
 ...
 ***************************************************************************/
char* CMseed2Gascii::getoptval(int argcount, char **argvec, int argopt, int dasharg)
{
 ...
 ***************************************************************************/
int CMseed2Gascii::readlistfile (char *listfile)
{
	FILE *fp;
	char  line[1024];
	char *ptr;
 ...
 ***************************************************************************/
listnode* CMseed2Gascii::addnode (struct listnode **listroot, void *key, int keylen,	void *data, int datalen)
{
	...
	if( key )
    {
		newlp->key = new char[keylen];// malloc (keylen);
		memset(newlp->key, 0x00, keylen);
		memcpy (newlp->key, key, keylen);
    }
  
	if( data)
    {
		newlp->data = new char[datalen];//malloc (datalen);
		memset(newlp->data, 0x00, datalen);
...
void CMseed2Gascii::usage (void)
{
	...
	fprintf(stderr,
					...
					"                3=Header followed by time-sample value & acc value pairs\n"
